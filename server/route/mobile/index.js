module.exports = app => {
    ...
    router.post('/Repair', authMiddle, async(req, res) => {
        ...
        result.data = results.splice(startid, 10)
        console.log(results)
        res.send(result)
    })
    ...
    router.post('/devicelist', authMiddle, async(req, res) => {
        ...
        let result = {}
        result.count = results.length;
        result.device_isonNum = 0;
        result.device_alarmNum = 0;
        results.forEach(element => {

            if (element.is_on === "1") {
                result.device_isonNum++;
            }
            if (element.status === "报警") {
                result.device_alarmNum++;
            }
    ...
    router.post('/AlarmRecord', authMiddle, async(req, res) => {
        ...
        let sql;
        if (req.user.read === "查看所有") {
            sql = `select a.*,d.device_name from alarm a left join device d  on a.device_eq=d.eq  where a.is_deleted=0  order by a.ts desc limit ${startid},10`
        } else {
            sql = `select a.*,d.device_name,d.enterprise_id from alarm a left join device d  on a.device_eq=d.eq  where a.is_deleted=0 and d.enterprise_id=${enterprise_id} order by a.ts desc limit ${startid},10`
        }
    ...
    router.post('/ModifyPassword', authMiddle, async(req, res) => {
        ...
        let sql = `select * from user_info u where u.username='${username}'  and is_deleted=0`
            // console.log(sql);
        let row = await connection(sql)
        assert(row.length, 422, '用户不存在')
        assert(row[0].password === md5(password), 422, '原用户密码不正确')
            // let results = await connection(sql)
            // console.log(results);
